 ...
 * 
 * Be careful when extending this class with more tests: A new SQLite database
 * file is created for <em>every</em> test.
 * 
...
public class TestSqliteURIBytesStorage {
    ...
    public void testInvalidTableName() throws Exception {
        ...
        JdbcURIBytesStorage invalidStorage = new JdbcURIBytesStorage("hi",
                tableName);
    ...
    public void testMultipleConnections() throws Exception {
        /**
         * Some testing with multiple connections to the same database with
        ...
        assertTrue(Arrays.equals(storage2.get("zhaw.ch"),
                "winterthur".getBytes()));
        ...
        for (int i = 0; i < MAX_I; i++) {
            for (int j = 0; j < MAX_J; j++) {
                ...
                Thread thrd2 = new Thread() {
                    ...
                    public void run() {
                        try {
                            JdbcURIBytesStorage myStorage = new JdbcURIBytesStorage(
                                    dbName, table);
        ...
        for (int j = 0; j < MAX_J; j++)
            assertTrue(Arrays.equals(storage.get("zhaw.ch/" + j),
                    "234".getBytes()));
    }
