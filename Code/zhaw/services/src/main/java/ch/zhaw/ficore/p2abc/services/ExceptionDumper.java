...
public class ExceptionDumper {
    ...
    private static String exceptionAsString(final Throwable t) {
    ...
    private static String htmlEntities(final String in) {
        if (in == null) {
            return "";
        }
    ...
    public static Response dumpException(final Throwable t, final Logger l) {
    ...
    public static String dumpExceptionStr(final Throwable t, final Logger l) {
        String asString = exceptionAsString(t);
        String hash = DigestUtils.sha1Hex(asString);
        l.warn("!!EXCEPTION!! " + hash + " " + asString);
